                ...
                <div className="p-4 space-y-2 text-center dark:text-white">
                    <h2 className="text-xl font-bold tracking-tight" id="page-action.heading">
                      {JSON.stringify(opponent)}
                        {!(winnerSymbol == me.symbol) ? "Opponent won :(" : "You won :)"}
                    </h2>
...
function TicTacToeBoard({ player, gameData }) {
  ...
  const makeMove = (row, col) => {
    ...
    const updatedBoard = board.map((r, i) =>
      r.map((val, j) => { newposition += (i === row && j === col ? currentSymbol : val) + ",";  return (i === row && j === col ? currentSymbol : val)})
    );
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 py-4">
      {winnerSymbol && <WinnerDetail winnerSymbol={winnerSymbol} opponent={gameData?.opponent} me={player} />}
      <h6 className="font-bold text-white">
        {(!winnerSymbol) && ((isMyTurn) ? "Your turn" : "Waiting for opponent to play")}
      </h6>
